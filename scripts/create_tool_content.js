// scripts/create_tool_content.js
// Generates data/toolContent.ts with structured educational formulas, steps, examples, assumptions, disclaimers, and FAQs for all 38 tools.
const fs = require('fs');
...
const outputContent = `// data/toolContent.ts
...
export const TOOL_CONTENT: Record<string, CalculatorContent> = ${JSON.stringify(toolContent, null, 2)};
...
const targetPath = path.resolve(__dirname, '../data/toolContent.ts');
fs.writeFileSync(targetPath, outputContent, 'utf-8');
console.log('Successfully created data/toolContent.ts with', Object.keys(toolContent).length, 'tools');
